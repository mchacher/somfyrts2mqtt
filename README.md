...
- **Device types** — a remote can be a roller shutter (default) or a **sliding gate ("portail")**. A gate is a single-button **toggle** (a sequential-mode motor cycles open/stop/close on one repeated RTS button) — driven by a `Toggle` command with a selectable button. Set per-remote in the admin UI ; the type is advertised on MQTT (`"Type":"gate"`) for the client to present accordingly. Additive and backward-compatible: existing shutters are untouched, no re-pairing.
