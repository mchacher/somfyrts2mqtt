...
- **Device types** — a remote can be a roller shutter (default) or a **sliding gate ("portail")**. A gate drops the time-based position and exposes Open / Close / Stop **plus a Toggle** — the dedicated Somfy RTS `Toggle` command (`0x0C`) that a single-button gate motor cycles on. Set per-remote in the admin UI ; the type is advertised on MQTT (`"Type":"gate"`) for the client to present accordingly. Additive and backward-compatible: existing shutters are untouched, no re-pairing.
